...
public class PlaylistsWorkflow {
    ...
    private final UsersPlaylistsRelationRepository userPlaylistRepository;
    private final UsersLikeplaylistsRelationRepository likeRelationRepository;
    ...
    public PlaylistActionVO createPlaylist(MultipartFile file, String name, String description,
        ...
        try {
            ...
            userPlaylistRepository.insert(relation);
    ...
    public PlaylistActionVO editPlaylist(MultipartFile file, Long playlistId, String name,
        ...
        try {
            ...
            playlistsRepository.updateById(updateEntity);
    ...
    public PlaylistContentVO getPlaylistContent(Long playlistId) {
        ...
        if (!relations.isEmpty()) {
            List<Long> songIds = relations.stream().map(SongsPlaylistsRelation::getSongId)
                    .toList();
            List<Songs> songEntities = songsRepository.findPlaylistSongs(songIds);
    ...
    public boolean isPlaylistLiked(Long playlistId, Long userId) {
        return userId != null && likeRelationRepository.countByUserAndPlaylist(userId, playlistId) > 0;
    }
    ...
    public void toggleLike(Long playlistId, Long userId, boolean isLike) {
        ...
        if (playlistsRepository.getById(playlistId) == null) {
            throw new BusinessException(404, "歌单不存在");
        }
        ...
        if (isLike) {
            ...
            if (likeRelationRepository.insertIgnore(relation) > 0) {
                playlistsRepository.incrementLikeCount(playlistId);
            }
            return;
        }
        int deleted = likeRelationRepository.deleteByUserAndPlaylist(userId, playlistId);
        if (deleted > 0) {
            playlistsRepository.decrementLikeCount(playlistId);
        }
    }
    ...
    public AddSongToPlaylistVO addSongToPlaylist(Long playlistId, Long songId, Long userId) {
        ...
        int nextPosition = songsPlaylistsRelationRepository.getMaxPosition(playlistId) + 1;
        ...
        try {
            songsPlaylistsRelationRepository.insert(relation);
        ...
        playlistsRepository.incrementSongCount(playlistId);
    ...
    public void removeSongFromPlaylist(Long playlistId, Long songId, Long userId) {
        assertPlaylistOwner(playlistId, userId);
        SongsPlaylistsRelation target = songsPlaylistsRelationRepository.findByPlaylistAndSong(playlistId, songId);

        if (target == null)
            throw new BusinessException(404, "歌曲不在该歌单中");

        songsPlaylistsRelationRepository.deleteByPlaylistAndSong(playlistId, songId);
        songsPlaylistsRelationRepository.decrementPositionsAfter(playlistId, target.getSongPlaylistPosition());

        playlistsRepository.decrementSongCount(playlistId);
    }

...
import com.kyf.mp.server.modules.queue.repository.impl.PlayStateRepositoryImpl;
import com.kyf.mp.server.modules.queue.repository.impl.QueueCustomRepositoryImpl;
import com.kyf.mp.server.modules.queue.repository.impl.QueueItemsRepositoryImpl;
import com.kyf.mp.server.modules.queue.repository.impl.QueuesRepositoryImpl;
...
class QueuesWorkflowTest {
    ...
    @BeforeEach
    void configureWorkflowDefaults() {
        queuesWorkflow = new QueuesWorkflow(new QueueCustomRepositoryImpl(queueCustomMapper),
                new PlayStateRepositoryImpl(playStateMapper),
                new QueueItemsRepositoryImpl(queueItemsMapper),
                new QueuesRepositoryImpl(queuesMapper),
                new PlaylistsRepositoryImpl(playlistsMapper),
                new SongsPlaylistsRelationRepositoryImpl(songsPlaylistsRelationMapper),
                new SongsRepositoryImpl(songsMapper));
        ReflectionTestUtils.setField(queuesWorkflow, "maxQueuesPerUser", 5);
    }
    ...
    void copiesPlaylistItemsThroughQueueItemMapper() {
        ...
        CreateQueueFromPlaylistVO result = queuesWorkflow.createQueueFromPlaylist(7L, 99L);
    ...
    void reinsertedSongClosesItsOldPositionBeforeInsertion() {
        ...
        queuesWorkflow.addSongToQueue(7L, 123L, request);
    ...
    void deletingItemBeforeCurrentSongSynchronizesPlaybackPosition() {
        ...
        queuesWorkflow.removeSongFromQueue(userId, queueId, deletedItemId);
    ...
    void addSongToQueueButSongNotExists() {
        ...
        BusinessException e = assertThrows(BusinessException.class, () -> {
            queuesWorkflow.addSongToQueue(songId, queueId, dto);
        });
